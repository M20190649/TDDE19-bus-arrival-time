...
\newline
Some additional pre-processing is required to train the ANN models. Most importantly the data needs to be labelled with the true labels used to train the models. Datasets aqcuired from the pre-processing described in section \ref{pre-processing} are complemented with the following columns:
...
\noindent The first two columns that are added are used as true labels in the respective models. The third one is a feature which captures conditions earlier in the journey. The specific features used by each model will now be described.
